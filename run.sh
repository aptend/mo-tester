...
while getopts ":p:m:t:r:i:e:s:ogfnh" opt
do
    case $opt in
        p)
        PATHC="path=${OPTARG}"
...
if [ ${TIMES} -eq 1 ]; then
  ...
  java -Xms1024M -Xmx1024M -cp ${WORKSPACE}:${libJars} \
          -Dconf.yml=${MO_YAML} \
          -Drun.yml=${RUN_YAML} \
          io.mo.Tester ${PATHC} ${METHOD} ${TYPE} ${RATE} ${INCLUDE} ${EXCLUDE} ${IGNORE} ${NOMETA} ${RESOURCE} ${FORCE} ${PPROF}
else
...
#        -Dconf.yml=${MO_YAML} \
#        -Drun.yml=${RUN_YAML} \
#        io.mo.Tester ${PATHC} ${METHOD} ${TYPE} ${RATE} ${INCLUDE} ${EXCLUDE} ${IGNORE} ${NOMETA} ${RESOURCE} ${FORCE}
}
