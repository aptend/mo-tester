...
public class ResultParser {
    ...
    private static ParseResult parseCommand(SqlCommand cmd, String text, TestScript script) {
        ...
        if (result == null || result.isEmpty()) {
            ...
            expResult.setOrginalRSText(result);
        }
    ...
    public static RSSet convertToRSSet(String rsText, String separator) {
        ...
        // Normalize different separator types to a unified separator
        String normalized = normalizeSeparators(rsText, separator);
        String[] lines = normalized.split("\n", -1);
        if (lines.length == 0) return null;
        
        // First line contains column headers
        String[] labels = lines[0].split(RESULT.COLUMN_SEPARATOR_SYSTEM, -1);
        RSSet rsSet = new RSSet();
        RSMetaData meta = new RSMetaData(labels.length);
        rsSet.setMeta(meta);
        for (String label : labels) {
            meta.addMetaInfo(label, label);
        }
        ...
        for (int i = 1; i < lines.length; i++) {
            ...
            rsSet.addRow(row);
            String[] values = lines[i].split(RESULT.COLUMN_SEPARATOR_SYSTEM, -1);
            for (int j = 0; j < labels.length; j++) {
                RSCell<String> cell = new RSCell<>();
    ...
    private static String normalizeSeparators(String text, String separator) {
        if ("both".equals(separator)) {
            ...
            return text.replaceAll(RESULT.COLUMN_SEPARATOR_SPACE, RESULT.COLUMN_SEPARATOR_SYSTEM)
                    .replaceAll(RESULT.COLUMN_SEPARATOR_TABLE, RESULT.COLUMN_SEPARATOR_SYSTEM);
        } else if ("table".equals(separator)) {
            // Replace only tab separators
            return text.replaceAll(RESULT.COLUMN_SEPARATOR_TABLE, RESULT.COLUMN_SEPARATOR_SYSTEM);
